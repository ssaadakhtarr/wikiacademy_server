...
exports.sendRoomDetails = function (req, res) {
  ...
  try {
    db.query(
        `INSERT INTO rooms (roomName, roomTitle, roomTagline, roomImage, roomDescription) VALUES (?, ?, ?, ?, ?)`,
        [roomName, roomTitle, roomTagline, roomImage, roomDescription],
        (err, result) => {
          if (err) {
            res.status(422).json({ status: "fail", message: err });
          } else {
            db.query(
              `INSERT INTO paths (roomsId) VALUES (?)`,
              [result.insertId],
              (err, result1) => {
                if (err) res.status(422).json({ status: "fail", message: err });
                else {
                    db.query(
                        `UPDATE paths SET tools = ?, web = ?, vuln = ?, beginner =? WHERE roomsId = ?`,
                        [
                          req.body.tools,
                          req.body.web,
                          req.body.vuln,
