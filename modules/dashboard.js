...
exports.getDashboard = function (req, res) {
  db.query(`SELECT COUNT(*) FROM users`, (err, results) => {
    db.query(
      ...
      (err, results1) => {
        const { rank, points, level, title, firstName, lastName, username,email,dateOfBirth, gender, occupation, areaOfInterest, summary, twitter, instagram, github, linkedin } = results1[0];
        res.send({
          ...
          lastName,
          username,
          email,dateOfBirth, gender, occupation, areaOfInterest, summary, twitter, instagram, github, linkedin,
        });
      }
    );
  });
